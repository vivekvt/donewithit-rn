...
class RegistrationScreen extends React.Component {
            ...
            <ScrollView>
                <View style={RegistrationStyles.container}>
                    ...
                    <View style={RegistrationStyles.emailinput}>
                        <TextInput style={RegistrationStyles.emailinput1}
                            placeholder={"NAME"}
                            value={this.state.registrationFormData.name}
                            onChangeText={(name) => { this.setState({ registrationFormData: { ...this.state.registrationFormData, name } }) }}
                        />
                    </View>
                    <View style={RegistrationStyles.emailinput}>
                        <TextInput style={RegistrationStyles.emailinput1}
                            ...
                            onChangeText={(mobile_no) => { this.setState({ registrationFormData: { ...this.state.registrationFormData, mobile_no } }) }}
                        />
                    </View>
                    <View style={RegistrationStyles.emailinput}>
                        <RNPickerSelect placeholder={{ label: 'SELECT A STATE' }} style={RegistrationStyles.emailinput1}
                            onValueChange={(state_id) => { this.setState({ registrationFormData: { ...this.state.registrationFormData, state_id } }) }}
                    ...
                    <View style={RegistrationStyles.emailinput}>
                        <RNPickerSelect placeholder={{ label: 'SELECT A DISTRICT' }}
                            ...
                            onValueChange={(district_id) => { this.setState({ registrationFormData: { ...this.state.registrationFormData, district_id } }) }}
                    ...
                    <View style={RegistrationStyles.emailinput}>
                        <TextInput style={RegistrationStyles.emailinput1}
                            ...
                            onChangeText={(city) => { this.setState({ registrationFormData: { ...this.state.registrationFormData, city } }) }}
                        />
                    </View>
                    <View style={RegistrationStyles.emailinput}>
                        <TextInput style={RegistrationStyles.emailinput1}
                            ...
                            onChangeText={(pin_code) => { this.setState({ registrationFormData: { ...this.state.registrationFormData, pin_code } }) }}
                        />
                    </View>
                    <View style={RegistrationStyles.emailinput}>
                        <TextInput style={RegistrationStyles.emailinput1}
                            ...
                            onChangeText={(email) => { this.setState({ registrationFormData: { ...this.state.registrationFormData, email } }) }}
                        />
                    </View>
                    <View style={RegistrationStyles.emailinput}>
                        <TextInput style={RegistrationStyles.emailinput1}
                            ...
                            onChangeText={(password) => { this.setState({ registrationFormData: { ...this.state.registrationFormData, password } }) }}
                        />
                    </View>
                    <View>
                        <RadioGroup radioButtons={[{ label: "Employee", value: "2" }, { label: 'Vendor', value: "3" }]}
                            onPress={(rolesMaster) => {
                                this.setState(() => {
                                    let userSelected = rolesMaster.find(e => e.selected == true);
                                    let selectedRole_id = this.state.rolesMaster.find(e => (e.label) == (userSelected.label)).value;
                                    return { registrationFormData: { ...this.state.registrationFormData, selectedRole_id } }
                                });
                            }}
                            flexDirection='row'
                        />
                    ...
                    <View style={[{ width: "76%", margin: 10, }]}>
                        <Button
                            title="SUBMIT"
                            color="#234191"
                            onPress={() => this.onPressRegister(this.state.registrationFormData)}>
                        </Button>
                    </View>
                </View>
            </ScrollView>
    ...
    addRolesToView() {
        onSuccess = (response) => {
            manupulateRolesMaster = response.data.data.map(function (roleElement) {
                ...
                { 
                    if (roleElement.label == 'Employee') {
                        ...
                        this.setState({ registrationFormData: { ...this.state.registrationFormData, role_id: roleElement.value } })
                    }else{
    ...
    addDistrictsToView() {
        onSuccess = (response) => {
            manupulateDistrictMaster = response.data.data.map(function (districtElement) {
                return { 'label': districtElement.name, 'value': districtElement.id };
            });
            this.setState({ isLoading: false, districtsMaster: manupulateDistrictMaster });
        };
        onFailure = (error) => { console.log(error) };
